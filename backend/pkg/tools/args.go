...
type WebSearchAction struct {
	...
	Mode    String `json:"mode,omitempty" jsonschema:"type=string,enum=links,enum=answer,enum=research,enum=exploit,enum=cert" jsonschema_description:"What you need back (default 'answer'). 'links' — a list of source links with snippets (fastest/cheapest). 'answer' — a synthesized answer over live sources. 'research' — deep multi-source analysis with reasoning (most thorough). 'exploit' — exploit code, PoCs, and offensive tools. 'cert' — certificate-transparency lookup for a DOMAIN: every issued subdomain, SAN and issuer from CT logs (query must be a domain like 'example.com' or '*.example.com'). web_search picks the concrete engine for you and falls back automatically; you do NOT name an engine."`
