...
-- Add shodan_internetdb to the searchengine_type enum (intel-platform fork: Shodan
-- free IP-intelligence engine, zero key, zero quota).
...
-- +goose Down
-- +goose StatementBegin
-- Revert the changes by removing shodan_internetdb from the enum
CREATE TYPE SEARCHENGINE_TYPE_NEW AS ENUM (
  'google',
  'tavily',
  'firecrawl',
...
-- Drop the new type and rename the new one
DROP TYPE SEARCHENGINE_TYPE;
ALTER TYPE SEARCHENGINE_TYPE_NEW RENAME TO SEARCHENGINE_TYPE;
